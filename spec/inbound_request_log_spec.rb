...
RSpec.describe InboundRequestLog do
  ...
  it "encrypts any passwords before storing" do
    ...
    request_log = InboundRequestLog.create(path: "/oauth/token/", request_body: request_body, method: 'POST', started_at: Time.current, uuid: SecureRandom.uuid)
  ...
  it "validates presence of uuid" do
    request_log = InboundRequestLog.new(path: "/oauth/token/", request_body: {}, method: "POST", started_at: Time.current)
    expect(request_log).not_to be_valid
  end
end
